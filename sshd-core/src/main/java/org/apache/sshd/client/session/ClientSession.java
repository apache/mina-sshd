...
public interface ClientSession
    ...
    default String executeRemoteCommand(String command) throws IOException {
        try (ByteArrayOutputStream stderr = new ByteArrayOutputStream()) {
            try {
                return executeRemoteCommand(command, stderr, StandardCharsets.US_ASCII, 0L);
            } finally {
                if (stderr.size() > 0) {
     ...
     */
    default String executeRemoteCommand(String command, OutputStream stderr, Charset charset, long commandTimeoutMillis) throws IOException {
        if (charset == null) {
            charset = StandardCharsets.US_ASCII;
        }

        try (ByteArrayOutputStream stdout = new ByteArrayOutputStream(Byte.MAX_VALUE)) {
            executeRemoteCommand(command, stdout, stderr, charset, commandTimeoutMillis);
     ...
     *
     * @param command              The command to execute - without a terminating LF.
     * @param stdout               Standard output stream - if {@code null} then stream data is ignored.
     *                             <b>Note:</b> if the stream is not {@code null}, it will be left <u>open</u> when this
     *                             method returns or an exception is thrown.
     * @param stderr               Error output stream - if {@code null} then error stream data is ignored.
     *                             <b>Note:</b> if the stream is not {@code null}, it will be left <u>open</u> when this
     *                             method returns or an exception is thrown.
     * @param charset              The charset to use for encoding the command and decoding the output/error streams.
     *                             If {@code null}, US-ASCII is assumed.
     * @param commandTimeoutMillis Timeout (in milliseconds) for the remote command execution. Applies to both
     *                             channel opening and result waiting. A zero or negative value means no timeout.
     * @throws IOException         If the command execution fails, times out, or returns a non-zero exit code.
     *                             A {@link RemoteException} may be thrown if the remote side reports an error.
    ...
    default void executeRemoteCommand(
            String command, OutputStream stdout, OutputStream stderr, Charset charset, long commandTimeoutMillis)
            throws IOException {
        if (charset == null) {
            charset = StandardCharsets.US_ASCII;
        }
        ...
        try (OutputStream channelErr = (stderr == null) ? new NullOutputStream() : new NoCloseOutputStream(stderr);
            ...
            Collection<ClientChannelEvent> waitMask = channel.waitFor(REMOTE_COMMAND_WAIT_EVENTS, commandTimeoutMillis);
