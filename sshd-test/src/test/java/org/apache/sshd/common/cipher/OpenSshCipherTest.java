...
class OpenSshCipherTest extends BaseTestSupport {
    ...
    static GenericContainer<?> sshdContainer = new GenericContainer<>(new ImageFromDockerfile()
            ...
            .withCopyFileToContainer(MountableFile.forClasspathResource(TEST_RESOURCES + "/sshd_bob.sh", 0x1ff),
                    "/entrypoint.sh")
            .waitingFor(Wait.forLogMessage(".*Server listening on.*port 22.*\\n", 1)) //
            .withExposedPorts(22) //
            .withLogConsumer(new Slf4jLogConsumer(LOG));

    private void initOpenSshCipherTest(String providerName, BuiltinCiphers factory, String name) {
    ...
    void connection(String providerName, BuiltinCiphers factory, String name) throws Exception {
        ...
        FileKeyPairProvider keyPairProvider = CommonTestSupportUtils.createTestKeyPairProvider(TEST_KEYS + "/user01_ed25519");
