...
public class SignatureSkED25519 implements Signature {

    private static final int FLAG_USER_PRESENCE = 0x01;

    private SkED25519PublicKey publicKey;
    ...
    public void initVerifier(SessionContext session, PublicKey key) throws Exception {
        if (!(key instanceof SkED25519PublicKey)) {
            throw new IllegalArgumentException("Only instances of SkED25519PublicKey can be used");
        }
        this.publicKey = (SkED25519PublicKey) key;
    ...
    public boolean verify(SessionContext session, byte[] sig) throws Exception {
        ...
        if (!SkED25519PublicKeyEntryDecoder.KEY_TYPE.equals(keyType)) {
            return false;
        }
        ...
        encoded.putString(KeyPairProvider.SSH_ED25519);
        ...
        Signature delegate = SecurityUtils.getEDDSASigner();
        delegate.initVerifier(session, publicKey.getEdDSAPublicKey());
