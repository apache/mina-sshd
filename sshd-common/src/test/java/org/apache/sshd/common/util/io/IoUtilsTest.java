...
public class IoUtilsTest extends JUnitTestSupport {
    ...
    public void testCheckExists() throws IOException {
        testCheckExists(Configuration.osX());
        testCheckExists(Configuration.unix());
        testCheckExists(Configuration.windows());
    }

    public void testCheckExists(Configuration config) throws IOException {
        try (FileSystem fs = Jimfs.newFileSystem(config)) {
            testCheckExists(fs);
        }
    }

    public void testCheckExists(FileSystem fs) throws IOException {
        Path baseDir = "\\".equals(fs.getSeparator()) ? fs.getPath("C:\\") : fs.getPath("/");
